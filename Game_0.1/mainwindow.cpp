    ...
    ui(new Ui::MainWindow)
{
    ...
    for(int x = 0; x < mp.layers.length(); x++){
        ...
        for(int z = 0; z < mp.layers.at(x).height; z++){
            ...
            int varX = 0;
            for(int y = 0; y < mp.layers.at(x).width;y++){
                QPoint point(varX,varY);


                    if(!mp.layers.at(x).tiles.at(durchgang).colision){

                        qDebug() << mp.layers.at(x).name;
                        if(mp.layers.at(x).tiles.at(durchgang).gid == 0 && mp.layers.at(x).name == "Spawner"){
                            goto identifier;
                        }
                        ...
                        if(mp.layers.at(x).name == "Spawner"){
                            spawner.append(DiTile);
                        }
    ...
    QBrush * brush = new QBrush(QColor(153, 204, 0),Qt::SolidPattern);
